<template>
  <div class="d-flex align-center">
    <div
      class="d-flex lightgrey align-center"
    ...
    >
      ...
      <v-chip
        ...
        small
        :close="
          !!item.value && !preSelectedItems.map((i) => i.key).includes(item.key)
        "
        class="ml-1"
      ...
      >
        ...
        <span v-if="!!item.value || item.operator" class="px-1">
          {{ item.operator || ":" }}</span
        >
    ...
    <v-btn
      ...
      class="ml-3"
      :disabled="
        filterItems.length < 1 || !!filterItems.find((f) => f.value === null)
      "
      :loading="loading"
      @click="submit"
    >
    ...
    <v-dialog v-model="showMap" width="1000">
      <div class="white">
        <client-only>
          <CoverageMap
            ref="map"
...
export default {
  ...
  computed: {
    ...mapState("staticCatalog", ["missions", "themes"]),
    availableItems() {
      return [
        {
          field_name: "theme",
        ...
        },
        {
          field_name: "mission",
          filter: "exact",
          available_values: this.missions
            ? this.missions.map((mission) => mission.name)
            : [],
        },
        {
          field_name: "type",
          filter: "exact",
          available_values: ["project", "product"],
        },
        {
          field_name: "bbox",
          filter: "bbox",
    ...
    isNumberField() {
      ...
      const currentField = this.availableItems.find(
        (f) => f.field_name === currentFilter.key
      );
      return currentField.digits !== undefined;
    },
    filterFields() {
      let items;
      const allItems = this.availableItems.filter(
        (f) =>
          f.filter &&
          (!this.filterItems.find((i) => i.key === f.field_name) ||
            (f.filter === "range" &&
              this.filterItems.filter((i) => i.key === f.field_name).length <
                2))
      );
      ...
      if (this.inProgressItem || this.currentlyFreeText) {
        const currentMeta =
          ...
          this.availableItems.find(
            (f) => f.field_name === this.inProgressItem.key
          );
        if (this.inProgressItem.operator) {
          ...
          if (currentMeta.available_values) {
            if (Array.isArray(currentMeta.available_values)) {
              items = currentMeta.available_values
                .map((i) => ({
                  filter_value: i,
                  field_name: i,
                ...
                .filter((item) => {
                  const previouslySetFilters = this.filterItems
                    ? this.filterItems.filter((i) => i.value)
                    : [];
                  const checkFilter = (type, check) => {
                    const applicableFilter = previouslySetFilters.find(
                      (f) => f.key === type
                    );
                    if (applicableFilter) {
                      const typeObject = `${type}s`;
                      const typeFilter = this[typeObject].find(
                        (t) => t.name === applicableFilter.value
                      );
                      if (typeFilter) {
                        const includedItem = typeFilter[check].find(
                          (v) => v.name === item.field_name
                        );
                        return includedItem;
                      }
                    }
                ...
                .sort((a, b) =>
                  a.field_name.localeCompare(b.field_name, "en", {
                    sensitivity: "base",
                  })
                );
            }
          ...
          const existingOperator = this.filterItems.find(
            (i) => i.key === this.inProgressItem.key && !!i.operator
          );
          items = [
  ...
  methods: {
    ...
    select(item) {
      if (item) {
        ...
        if (this.currentlyFreeText) {
          this.filterItems = this.filterItems.map((i) => {
    ...
    onDelete() {
      ...
      if (
        this.preSelectedItems
          .map((i) => i.key)
          .includes(this.filterItems[this.filterItems.length - 1].key)
      ) {
        return;
      }
      ...
      try {
        ...
        const searchQuery = this.filterItems.reduce((acc, curr) => {
          ...
          if (keywordKeys.includes(curr.key)) {
            filterQuery += `${
              filterQuery.length > 0 ? " AND " : ""
            }keywords ILIKE '%${curr.key}:${curr.value}%'`;
          }
          return curr.key === "type"
            ? `${acc}&type=${
                curr.value.toLowerCase() === "project"
                  ? "datasetcollection"
                  : "dataset"
              }`
        ...
        const queryString = `/collections/metadata:main/items?limit=12&sortby=${
          this.sortOrder === "Descending" ? `-${this.sortBy}` : `${this.sortBy}`
        }&offset=${(this.currentPage - 1) * 12}${searchQuery}${
          filterQuery ? `&filter=${filterQuery}` : ""
        }${this.bbox ? `&bbox=${this.bbox.join(",")}` : ""}`;

        await this.fetchCustomQuery(queryString)
            ...
            if (this.paginationLoop) {
              ...
              for (
                ...
                await this.fetchCustomQuery(
                  `/collections/metadata:main/items?limit=12&sortby=${
                    this.sortOrder === "Descending"
                      ? `-${this.sortBy}`
                      : `${this.sortBy}`
                  }&offset=${(currPage - 1) * 12}${searchQuery}${
                    filterQuery ? `&filter=${filterQuery}` : ""
                  }${this.bbox ? `&bbox=${this.bbox.join(",")}` : ""}`
                )
                  .then((response) => {
    ...
    handleDraw(newBbox) {
      ...
      const alreadySetIndex = this.filterItems.findIndex(
        (i) => i.key === "bbox"
      );
