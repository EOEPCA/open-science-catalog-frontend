<template>
  <div v-if="theme">
    ...
    <div
      ref="themeBanner"
      :style="`backgroundImage: url('${$metadataApi.defaults.baseURL}/themes/${theme.assets.image.href}');
...
export default {
  ...
  async created () {
    this.theme = await this.$metadataApi.$get(`/themes/${this.$route.params.theme}`)
    const allThemes = await this.$metadataApi.$get('/metrics')
    ...
    await Promise.all(this.theme.links.map(async (link) => {
      if (link.rel === 'item') {
        const projectResponse = await this.$metadataApi.$get(`/themes/${link.href.slice(0, -5)}`)
