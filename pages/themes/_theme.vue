<template>
  <div v-if="theme">
    ...
    >
      <v-tabs v-model="tab" background-color="#003247" dark grow>
        ...
        <v-tabs-items v-model="tab">
          <v-tab-item>
            ...
            <v-row class="pt-8 d-flex align-center">
              <v-col cols="12" md="4">
                <span class="text-h4"> Variables </span>
              </v-col>
              ...
              <v-col cols="12" md="8" class="d-flex">
                ...
                <v-select
                  ...
                  @change="
                    orderData(
                      'projects',
                      projectsDetailsFilter.toLowerCase(),
...
export default {
  ...
  head() {
    return {
      title: this.$route.params.theme.replace(
        /(^\w{1})|(\s+\w{1})/g,
  ...
  async created() {
    const theme = await this.$store.dispatch(
      "staticCatalog/retreiveTheme",
    ...
    await Promise.all(
      theme.links.map(async (link) => {
        if (link.rel === "item") {
          const projectResponse = await this.$staticCatalog.$get(
            this.$replaceStaticBase(link.href)
          );
    ...
    this.orderData(
      "projects",
      this.projectsDetailsFilter,
      this.projectsDetailsOrder,
      this.projectsSearch,
      true
    );
    this.orderData(
      "variables",
      "name",
      this.variablesDetailsOrder,
      this.variablesSearch
    );
  },
  methods: {
    ...
    orderData(source, key, direction, string, nested = null) {
      ...
      if (source === "variables") {
        ...
        this.variablesDetails = string
          ? this.filterByValue(sortedSource, string)
          : sortedSource;
      } else {
        const sortedSource = this.projectDetailsRaw.sort(compare);
        this.projectDetails = string
          ? this.filterByValue(sortedSource, string, "properties")
          : sortedSource;
      }
    },
    ...
    async handleProjectEmit(result) {
      await Promise.all(
        result.items
          .filter((project) => this.$extractSlug(project))
          .map(async (project) => {
            await this.retreiveProjects(this.$extractSlug(project))
              .then((projectResponse) => {
                project.links = projectResponse.links;
                project.properties["osc:consortium"] =
                  projectResponse.properties["osc:consortium"];
              })
              .catch((err) => {
                console.error(err);
              });
    ...
    handleVariableEmit(result) {
      ...
      this.variablesDetails = this.variablesDetailsRaw.filter(
        (variable) => resultedVariables[variable.name]
      );
    },
  },
};
