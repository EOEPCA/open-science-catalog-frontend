<template>
  <v-container :class="$vuetify.breakpoint.lgAndUp ? 'px-15 pt-8' : 'pa-4'">
    <h2 class="text-h2 mt-3 mb-5">Contributions status</h2>
    <v-row class="d-flex justify-center text-h6">
      <v-col cols="12">
        <v-icon> mdi-information-outline </v-icon>
        Note: only your most recent contributions are shown in this list. For a
        full list of contributions, please refer to the
        <a
          href="https://github.com/EOEPCA/open-science-catalog-metadata/pulls"
          target="_blank"
          style="text-decoration: none"
        >
          Open Science Catalog Metadata GitHub Repository </a
        >.
      </v-col>
    </v-row>
      ...
      <v-col cols="12">
        ...
        >
          ...
          <template #item.state="{ item }">
            <v-chip :color="getStatusStyle(item.state).color" small dark>
              <v-icon small left>
          ...
          <template #item.change_type="{ item }">
            <v-btn color="black" plain small class="px-0">
              <v-icon small left>
